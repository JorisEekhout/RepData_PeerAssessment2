...
```{r availableData, cache=TRUE, echo=TRUE}
...
In total `r sumAvailableData` data entries are used in this analysis. This is `r percentageAvailableData`% of the total number of data entries.
...
```{r changeEVTYPE, cache=TRUE, echo=TRUE}
...
```{r changeDMG, cache=TRUE, echo=TRUE}
...
```{r eventHealth, cache=TRUE, echo=TRUE}
library(dplyr)
...
```{r eventEconomy, cache=TRUE, echo=TRUE}
library(dplyr)
